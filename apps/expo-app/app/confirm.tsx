...
export default function ConfirmScreen() {
    ...
    try {
      ...
      if (password) {
        try {
          ...
          Alert.alert('Confirmed', 'Account confirmed successfully. Please sign in.');
          router.replace('/login');
          return;
        }
      } else {
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={{ paddingTop: 40 }}>
      ...
      <TextInput style={styles.input} placeholder="Confirmation Code" value={confirmationCode} onChangeText={setConfirmationCode} keyboardType="numeric" />
      <TouchableOpacity style={[styles.button, loading ? { opacity: 0.6 } : {}]} onPress={handleConfirm} disabled={loading}>
        <Text style={styles.buttonText}>{loading ? 'Processing...' : 'Confirm and Sign In'}</Text>
      </TouchableOpacity>
